...
def sparse_autoencoder_error(y_true, y_pred):
    return torch.mean(torch.square(torch.mul(torch.sign(y_true),torch.Tensor((y_true.detach().numpy()-y_pred))))).detach().numpy()
...
if __name__ == '__main__':
    ...
    begin = time.time()
    # 训练
    m = DHNE(dim_feature=args.dim_feature, embedding_size=[16, 16, 16],hidden_size=64)
    # 定义优化器
    ...
    for i in range(epoch):
        ...
        gen = train_dataset.next_batch2(dataset.embeddings,batch_size=16,num_neg_samples=args.num_neg_samples)
        while gen is not None:
            gen = list(gen)
            decode0, decode1, decode2, classify_res = m(torch.Tensor(gen[0]['input_0']), torch.Tensor(gen[0]['input_1']), torch.Tensor(gen[0]['input_2']))
            target_d0, target_d1,target_d2,target_classify_res = gen[1]["decode_0"],gen[1]["decode_1"],gen[1]["decode_2"],gen[1]["classify_layer"]
            # 训练过程中的输入与输出
        ...
        with torch.no_grad():
            ...
            gen = test_dataset.next_batch2(dataset.embeddings, batch_size=16, num_neg_samples=args.num_neg_samples)
            while gen is not None:
                gen = list(gen)
                ...
                if classify_res == target_classify_res:
                    acc += 1
                ...
                # 或者分类层的loss
                loss2 =  (
                    nn.functional.binary_cross_entropy(train_data3, test_data3.float())) + args.alpha * sum(loss1)
                ...
                print("roc_auc_score:",roc_auc_score(np.array(test_data[3]),np.array(train_data[3].data.numpy().reshape(1,train_data[3].size(dim=0))[0])))
            acc = acc / test_dataset.nums_examples
            print("acc:", acc)
    ...
    # ac = np.load('/Users/yizhihenpidehou/Desktop/fdu/eg/DHNE/result/GPS/model_16/embeddings.npy',allow_pickle=True)
