...
class hypergraph(object):
    ...
    def save_embeddings(self, dataset, file_name='embeddings'):
        emds = self.get_embeddings(dataset)
        prefix = '{}_{}'.format(self.options.prefix_path, self.options.embedding_size[0])
        prefix_path = os.path.join(self.options.save_path, prefix)
        if not os.path.exists(prefix_path):
            os.makedirs(prefix_path)
        for index,e in enumerate(emds):
            np.save(open(os.path.join(prefix_path, file_name+str(index)+'.npy'), 'wb'), e)
...
if __name__ == '__main__':
    ...
    print("embedding_size:",dataset.embeddings[0].shape,dataset.embeddings[1].shape,dataset.embeddings[2].shape)
    print("predict:",h.predict(embeddings=dataset.embeddings,data=dataset.test.edge))
    K.clear_session()
...
#     print("predict_res:",h.predict(embedding,data=dataset))
#
